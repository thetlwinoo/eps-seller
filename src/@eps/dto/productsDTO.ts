...
import { StockItems, Photos } from '@eps/models';
import { StockItemsDTO } from '@eps/dto';
...
export class ProductsDTO {
  ...
  constructor(product?) {
    ...
    this.productAttributeList = product.stockItemLists
      ? RootUtils.getUnique([...new Set(product.stockItemLists.map(item => item.productAttribute))], 'id')
      : [];
    this.productOptionList = product.stockItemLists
      ? RootUtils.getUnique([...new Set(product.stockItemLists.map(item => item.productOption))], 'id')
      : [];
        ...
        this.productOptionList.forEach(option => {
          if (attribute && option) {
            ...
            const item = this.stockItemLists
              ? this.stockItemLists.find(x => x.productAttribute.id == attribute.id && x.productOption.id == option.id)
              : null;
